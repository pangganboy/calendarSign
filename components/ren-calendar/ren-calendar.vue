			...
			<view :style="{ top: positionTop + 'rpx' }" class="days">
				<view class="item" v-for="(item, index) in dates" :key="index">
					<view class="day" @click="selectOne(item, $event)" :class="{
							...
							markDay:isMarkDay(item.year, item.month, item.date)&&item.isCurM,
							isBigWeek:item.isBigWeek,
							ifAfter:ifAfter(item)
                        }">
						{{ Number(item.date) }}
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 点击回调
			selectOne(i, event) {
				if (this.isMarkDay(i.year, i.month, i.date) && i.isCurM) return;
				let date = `${i.year}-${i.month}-${i.date}`;
				let selectD = new Date(date).getTime();
				let curTime = new Date().getTime();
